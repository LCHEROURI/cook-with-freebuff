...
**Date**: 2026-08-26
**Status**: In Progress (the Phase 1 track sqlconnect_phase1_20260826 is complete: provisioning, the schema and operations port, the emulator proof, and the concurrency contract test landed via PRs #182, #184, #185, and #187; the running app still uses Firestore, and repository parity, cutover, and decommission are not started)
...
# shape, same collection) uses active/recurringCount/signature/weeks/runUrl,
# and every doc's ranAt maps onto recordedAt (no separate ranAt column).
...
type DeployStatus @table(key: "slot") {
  ...
  weeks: [String!]                # flake_streak streak weeks rendered by /status
  runUrl: String
}
...
`@check` message; the SQL Connect repository is not written yet, and the
future repository must map that message back to the existing
...
The flake_streak shape (active/recurringCount/signature/weeks/runUrl) lives in
nullable columns on the same row type, and each doc's `ranAt` maps onto
`recordedAt`. The `weeks` array (the streak weeks `/status` renders) was added
during the Phase 2 port after code review caught the scope missing it.
...
- `mutations.gql`: inserts/updates/deletes per entity (upsert for
  `dietaryProfile` and `deployStatus`), the session update mutations from
  decision 3, and the timer rebase as a single Native SQL `_execute`
  (`UPDATE cooking_timers SET ends_at = ends_at + ($1 * interval '1 millisecond')
  WHERE session_id = $2 AND status = 'RUNNING'`), one atomic statement in
  Postgres replacing the Firestore batch. GraphQL cannot express this: each
  row's shift needs `Timestamp_Duration`, which SQL Connect forbids as a
  variable, so the offset is applied as `$offsetMs` times
  `interval '1 millisecond'` in raw SQL. The local emulator cannot run
  parameterized native DML (a PGLite defect), so the rebase is proven by
  `dataconnect:compile` and will be exercised against Cloud SQL in the
  parity phase.
...
- `connector.yaml`: `adminNodeSdk` (the current firebase-tools key; older
  docs call it `nodeAdminSdk`) output to `lib/server/dataconnect`, package
  `@cook-with-freebuff/dataconnect-admin` (the SDK output is gitignored and
  regenerated by `dataconnect:compile`).
