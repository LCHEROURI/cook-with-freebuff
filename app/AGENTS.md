...
- Route handlers are thin: parse → validate → call a `lib/server` service or tool → return `{ success: true, data }` or `{ success: false, error: { code, message, recoverable } }`. This envelope applies to the dispatch routes (`cook`, `kitchen`, `tools`) and the model endpoints (`voice/token`, `vision/scan`), whose consumers read `body.data.token` / `body.data.ingredients`; `agent` returns the `AgentTurn` directly (the voice hook reads that shape), and `status`/`build-info` return their top-level fields bare — do not wrap those in `data` (spec 0001).
