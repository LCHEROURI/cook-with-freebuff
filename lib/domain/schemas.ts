...
/**
 * The user-provided build constraints a recipe was created for ("for 4, no
 * peanuts, vegetarian" → servings 4, allergies [peanuts], dietaryRestrictions
 * [vegetarian]). Optional on the recipe — recipes generated by the agent path
 * (or created before this field existed) carry none.
 */
export const recipePreferencesSchema = z.object({
  // `.default(null)` keeps the OUTPUT type `number | null` (matching
  // RecipePreferences) while still accepting an omitted/null input — the
  // starter prompt may carry no serving count.
