// ─────────────────────────────────────────────────────────────────────────────
// Gemini tool declarations + Live system instruction (SDK-free)
//
// Single source of truth for the tool surface the model sees. Pure JSON — no
// SDK imports — so it can be imported by BOTH the server conversation agent
// (lib/ai/conversation.ts) and the browser Gemini Live client
// (lib/voice/gemini-live.ts) without pulling the Google SDK into the client
// bundle. The Live session and /api/agent therefore expose the SAME tools.
// ─────────────────────────────────────────────────────────────────────────────
...
/**
 * The standing rules the Kitchen Agent follows. Shared verbatim by the
 * server conversation agent (via buildConversationPrompt) and the Gemini
 * Live session's systemInstruction — one text, two transports, zero drift.
 */
...
/**
 * The Live session's system instruction, including what the orchestrator
 * would normally put in the prompt context (current phase/step/timers).
 */
export function buildLiveSystemInstruction(context: {
  currentPhase?: string;
  currentStep?: string;
