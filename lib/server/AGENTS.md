...
- No raw Firestore in services: reads/writes go through the repository layer (`repositories.ts`). Full writes are strictly Zod-validated before Firestore I/O; partial writes are strictly parsed, merged with the stored document, checked against the complete schema and immutable-field policy, then persisted as a sanitized patch. Validation is a repository duty, not a caller duty.
