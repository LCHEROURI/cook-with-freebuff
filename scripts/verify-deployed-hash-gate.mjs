...
//
// Reports the commit the Firebase App Hosting host is CURRENTLY serving and
// compares it against local HEAD, so you know what you are about to change
// before any deploy. It composes the existing shared driver
// (scripts/verify-deployed-hash.mjs) rather than reimplementing its
// machinery — the same tokenless /api/build-info resolution and exit-code
// contract every other gate uses, so the local gate and the post-deploy gate
// can never disagree about what "the live commit" means.
//
//   1. Resolves local HEAD (git rev-parse HEAD).
//   2. Runs verify-deployed-hash.mjs with//   --url https://cook-with-freebuff--portfolio-app-freebuff2.us-central1.hosted.app
//       (the live production host — public, no token needed)
//       --expect <local HEAD>
//     which prints the live commit and asserts the served sha matches local
//     HEAD.
...
//                                                      # CI PR gate
//
// Read-only against the host and git; no source changes.
// ============================================================================

import { spawnSync } from 'node:child_process';
import { resolve } from 'node:path';

export const CANONICAL_URL = 'https://cook-with-freebuff--portfolio-app-freebuff2.us-central1.hosted.app';
...
// ── 2. Run the shared hash driver against the live production host ──────────
...
if (!live) {
  // No live sha (offline, host error, or no rollout yet): we cannot make
...
if (anc.status === 0) {
  console.log(`\n  ✓ live (${live.slice(0, 12)}…) is behind ${headLabel} — forward deploy; the post-deploy gate verifies after App Hosting finishes`);
