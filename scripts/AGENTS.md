...
| `codex-review-pr-gate.mjs` | Required PR check: skips Codex polling for low-risk docs/policy/tests-only PRs; scans high-risk PR findings, blocks open P0/P1, certifies bot skips, and rescues same-head wait expiries from shadowing an already-green sibling run per the conventions below |
...
- Low-risk docs/policy/tests-only PRs do not wait for Codex and do not consume Codex credits. For high-risk PRs, if the Codex bot skips (no review within the wait window), certify with the `CODEX_GATE_BOT_SKIPPED_PRS` repo variable listing the PR number, then push a synchronize event; delete the variable after merge. Automatic nudge commits are disabled in the workflow; `CODEX_GATE_NUDGE_MAX` is a local emergency opt-in only.
...
- An empty Codex comment list is NOT a clean review for high-risk PRs: the gate's WAITING fallback blocks until the bot reviews or a human certifies a bot skip. Low-risk PRs exit before this polling path.
- The production workflow does not receive `CODEX_NUDGE_TOKEN` and sets `CODEX_GATE_NUDGE_MAX=0`, so it cannot create repeated credit-consuming nudge commits. The script retains an explicit local override only for a human-approved emergency retry.
