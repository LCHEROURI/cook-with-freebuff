...
| `tidy-branches.mjs` | One-command branch tidy: prunes stale remote-tracking refs (`git remote prune origin`) and deletes only fully merged local branches (`git branch -d`, which git refuses for unmerged and squash-merged branches — a platform check precedes any `-D`); `--dry-run` previews; pinned by `tidy-branches.test.ts` |
...
- `deploy_status/verify_live` is single-slot: the recorder overwrites the fixed doc on every run, so the status page always shows the newest verification — a newer deploy's verify replaces the previous record (the record step only runs when verify:live actually ran, so a skipped pre-flight never overwrites the last real result).
